...
<div align=right>
View:&nbsp;<a href="instructions.shtml"><span style="background : black; color : white;">&nbsp;Instructions&nbsp;</span></a>|<a href = "CalendarView.shtml" target="_top"><span style="background : black; color : white;">&nbsp;Regular View&nbsp;</span></a>&nbsp;</div>
<div align=center>
...
<form name="form2" action="Remove.asp" method="post" onSubmit="return fCheck();">
...
'---- Write day of month
	...
	%>
	<a target ="left" href=admin.asp?day=0<%= Day(dtCurViewDay) %>&month=0<%= month(dtCurViewMonth) %>&year=<%= year(dtCurViewMonth) %>><font color=black><%= Day(dtCurViewDay) %></font></a>
	...
	if ubound(all_events) = ubound(all_ids)then
	  For K = lbound(all_events)+1 To ubound(all_events)	
		 if Instr(all_events(K), "--continued") = 0  then
		 	response.write "<table cellpadding=1 cellspacing=0><tr><td valign=top><input type='checkbox' name='txtDel" & rowcount & "' value = '"  & all_events(K) & "'></td><td valign=top><span id = time><font color=gray>" & all_times(K) & "</span>&nbsp;" 
	%>
	  		<font id=id><a href ="javascript:Pop('detailsadmin.asp?id=<%= all_ids(K) %>', 210, 550)"><%= all_events(K) %></a></font></td></tr></table>
<%
		else response.write "<table cellpadding=1 cellspacing=0><tr><td valign=top align=right><span id=time>"& all_times(K) & "</span></td><td valign=top><span id = id3>" & all_events(K) & "</span></td></tr></table>"
		end if
		 rowcount=rowcount+1
	  Next
	end if
...
<input type="hidden" name="mm" value="<%= mnth %>">
<input type="hidden" name="yyyy" value="<%= tmpyear %>"><br>

<% if rowcount > 0 then %>
<a href="javascript:fCheckAll()">Check All</a> | <a href="javascript:fClearAll()">Clear All
<div align = center>
<input type="submit" name="submit" value="Remove Checked Items"> 
</div>
<% end if %>
